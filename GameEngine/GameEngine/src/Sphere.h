...
private:
	...
	void RenderSphereVertex(float radius, unsigned int sectors, unsigned int stacks, std::vector<Vertex>& vertices, std::vector<unsigned int>& indices);
public:
	Sphere(float radius, unsigned int sectors, unsigned int stacks);
	const std::vector<float>& GetVertices(void) const;
	const std::vector<Vertex>& GetVerticesVertex(void)const;
